...
namespace {
constexpr size_t kStateInterfacesPerJoint = 3;
}
...
CallbackReturn RobotJointControllerGroup::on_activate(const rclcpp_lifecycle::State &previous_state)
{
    ...
    {
        const size_t base = static_cast<size_t>(index) * kStateInterfacesPerJoint;
        double init_pos = state_interfaces_[base].get_value();
...
void RobotJointControllerGroup::UpdateFunc(const double &period_seconds)
{
    ...
    for (int index = 0; index < joint_names_.size(); ++index)
    {
        ...
        const size_t base = static_cast<size_t>(index) * kStateInterfacesPerJoint;
        currentPos[index] = state_interfaces_[base].get_value();
        currentVel[index] = state_interfaces_[base + 1].get_value();
        ...
        last_state_.motor_state[index].tau_est = state_interfaces_[base + 2].get_value();
    }
...
{
    if (index < 0 || static_cast<size_t>(index) >= joints_urdf_.size() || !joints_urdf_[index] || !joints_urdf_[index]->limits)
    {
        RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(), 2000,
            "URDF limits not ready for joint index %d; skipping position limit", index);
        return;
    }
...
void RobotJointControllerGroup::VelocityLimit(double &velocity, int &index)
{
    ...
    {
        RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(), 2000,
            "URDF limits not ready for joint index %d; skipping velocity limit", index);
        return;
    }
...
void RobotJointControllerGroup::EffortLimit(double &effort, int &index)
{
    ...
    {
        RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(), 2000,
            "URDF limits not ready for joint index %d; skipping effort limit", index);
        return;
    }
