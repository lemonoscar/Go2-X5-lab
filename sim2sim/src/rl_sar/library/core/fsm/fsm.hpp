...
public:
    ...
    {
        if (initial_state_name_.empty())
        {
            std::cout << LOGGER::WARNING << "[FSM] Initial state is not set; reset skipped." << std::endl;
            return;
        }

        const auto it = states_.find(initial_state_name_);
        if (it == states_.end())
        {
            std::cout << LOGGER::ERROR << "[FSM] Initial state '" << initial_state_name_ << "' not found!" << std::endl;
            return;
        }
        ...
        mode_ = Mode::NORMAL;
        current_state_->Enter();
        std::cout << LOGGER::INFO << "[FSM] Reset to initial state: " << initial_state_name_ << std::endl;
    }
