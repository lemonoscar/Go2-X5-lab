...
struct Observations
{
    ...
    std::vector<T> arm_joint_command;
    std::vector<T> gripper_command;
};

struct RLCommandOutput
{
...
public:
    ...
    tbb::concurrent_queue<RLCommandOutput> output_cmd_queue;
    ...
    void ResetEpisodeState(bool reset_control = true);
    ...
    std::vector<float> PadActionsForObservation(const std::vector<float>& actions) const;
    std::vector<float> ApplySim2SimActionTransform(const std::vector<float>& policy_actions);
    void ResetActionRuntimeState();

    // yaml params
    void ReadYaml(const std::string& file_path, const std::string& file_name);
    virtual void ValidateLoadedPolicyConfig();
    void ValidatePolicyManifest(const std::string& robot_config_path, const std::string& model_path);
    ...
    std::vector<float> last_policy_actions;
    std::vector<float> last_applied_actions;
    std::deque<std::vector<float>> sim2sim_action_history;
    int sim2sim_action_delay_steps = 0;
    std::mt19937 sim2sim_rng{std::random_device{}()};
...
protected:
    ...
    virtual void ResetSimRuntimeState() {}
    bool go2_x5_fixed_cmd_pending = false;
};
