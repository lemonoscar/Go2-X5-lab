...
run_ros_build() {
    ...
    if [[ "$ROS_DISTRO" == "foxy" || "$ROS_DISTRO" == "humble" ]] && [ -x /usr/bin/python3 ]; then
    ...
    if [ ${#packages[@]} -eq 0 ]; then
        if [[ "$ROS_DISTRO" == "noetic" ]]; then
            ...
            if [ ${#cmake_args[@]} -gt 0 ]; then
                colcon build --merge-install --symlink-install --cmake-args "${cmake_args[@]}"
            else
        ...
        if [[ "$ROS_DISTRO" == "noetic" ]]; then
            ...
            print_info "Building specific packages: $package_list"
            if [ ${#cmake_args[@]} -gt 0 ]; then
                colcon build --merge-install --symlink-install --packages-select $package_list --cmake-args "${cmake_args[@]}"
            else
