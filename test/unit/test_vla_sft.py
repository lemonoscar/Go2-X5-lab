...
VLA_SFT_PARENT = REPO_ROOT / "source" / "robot_lab" / "robot_lab" / "tasks" / "manager_based" / "manipulation"
sys.path.insert(0, str(VLA_SFT_PARENT))
...
def test_scene_registry():
    ...
    from vla_sft.scenes import BasicGraspSceneRegistry
...
def test_mobile_scene_registry():
    ...
    from vla_sft.scenes import MobileGraspSceneRegistry
...
def test_scene_config():
    ...
    from vla_sft.scenes import BasicGraspSceneA1
...
def test_mobile_scene_config():
    ...
    assert any(verb in instruction.lower() for verb in ("navigate", "go", "move"))
...
def test_instruction_generator():
    ...
    from vla_sft.data_collection import InstructionGenerator
    from vla_sft.scenes import BasicGraspSceneA2, MobileGraspSceneB1
...
def test_scene_manager():
    ...
    from vla_sft.data_collection import VLASSceneManager
