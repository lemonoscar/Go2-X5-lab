#!/bin/bash
set -euo pipefail
# Test with arm camera elevated (looking forward from above gripper)
...
exec "${ISAACLAB_ROOT}/isaaclab.sh" \
  -p "${ROOT_DIR}/scripts/visualization/visualize_camera.py" \
  --dog_cam_pos 0.30 0.0 0.16 \
  --dog_cam_rot -0.3799 0.5963 0.5963 -0.3799 \
  --arm_cam_pos 0.08657 0.0 0.03 \
  --arm_cam_rot 0.271 0 0 0.962 \
  --save_images \
  "$@"
