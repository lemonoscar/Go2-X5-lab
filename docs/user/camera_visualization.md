...
```bash
python scripts/visualization/visualize_camera.py [选项]
```
...
| 脚本 | 描述 | 臂相机朝向 |
|------|------|-----------|
| `test/startup/camera/test_cam_current.sh` | 当前配置 | 向后看（180°旋转） |
| `test/startup/camera/test_cam_forward.sh` | 推荐 | 向前看 |
| `test/startup/camera/test_cam_downward.sh` | 俯视配置 | 向下45° |
| `test/startup/camera/test_cam_elevated.sh` | 抬高配置 | 向前下30°，位置上移3cm |

### 3. 运行示例

```bash
# 测试当前配置（在Isaac Sim中可视化）
./test/startup/camera/test_cam_current.sh

# 测试推荐配置（相机向前看）
./test/startup/camera/test_cam_forward.sh

# 保存相机图像到文件
./test/startup/camera/test_cam_forward.sh --save_images

# 无头模式运行（不显示GUI，只保存图像）
./test/startup/camera/test_cam_forward.sh --save_images --no_render

# 自定义参数测试
python scripts/visualization/visualize_camera.py \
