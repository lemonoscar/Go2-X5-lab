...
"""
Environment configuration for VLA-SFT Layer 2 (Mobile Grasp) data collection.

This module extends the basic grasp environment with:
- Base navigation (4-DOF: x, y, yaw, and optional height)
- Arm manipulation (6-DOF)
- Gripper control (2-DOF binary)
- Path planning and obstacle avoidance
- Table approach scenarios
"""
...
# Import MDP functions from basic layer
from ..mdp import (
...
@configclass
class Go2X5VLASMobileSceneCfg(InteractiveSceneCfg):
    """Scene configuration for VLA-SFT Mobile Grasp environments.

    Extends the basic grasp scene with:
    - Dynamic target positioning (1.5-3m from robot)
    - Optional table for approach scenarios
    - Obstacles for navigation tasks
    - Occluders for partial occlusion tasks
    """

    robot: ArticulationCfg = MISSING

    # Target object (can be placed at various distances)
    ...
    # Obstacles (for B3: obstacle avoidance)
    obstacles: List[RigidObjectCfg] = field(default_factory=list)

    # Occluder (for B4: partial occlusion)
    occluder = AssetBaseCfg(
        prim_path="/World/Occluder",
        spawn=None,  # Disabled by default, enabled for B4 scenes
    )

    # Table surface (for B2: table approach)
    table = AssetBaseCfg(
        prim_path="/World/Table",
        spawn=None,  # Disabled by default, enabled for B2 scenes
    )
    ...
    # Dog camera (body-mounted) - wider FOV for navigation
...
@configclass
class MobileActionsCfg:
    """Action configuration for Go2-X5 VLA-SFT Mobile Grasp.

    Actions include:
    - Base velocity commands (x, y, yaw)
    - Arm joint control
    - Gripper control
    """

    # Base velocity commands (for navigation)
    base_velocity = JointPositionActionCfg(
        asset_name="robot",
...
@configclass
class MobileObservationsCfg:
    """Observation configuration for mobile grasp policy learning."""
...
@configclass
class MobileEventCfg:
    """Event configuration for mobile scene randomization."""
    ...
    reset_object_position = EventTerm(
        ...
        params={
            "pose_range": {
                "x": (0.5, 3.0),  # Extended range for mobile grasp
                "y": (-1.5, 1.5),
                "z": (0.0, 0.0),
                "roll": (0.0, 0.0),
                "pitch": (0.0, 0.0),
...
@configclass
class MobileRewardsCfg:
    """Reward configuration for mobile grasp training."""

    # Navigation rewards
    approaching_target = RewTerm(
        func=object_ee_distance,
        params={"std": 0.5},  # Larger std for navigation phase
        weight=1.5,
    )
    ...
    stable_base = RewTerm(
        ...
        weight=1.5,  # Higher weight for mobile scenarios
    )
    ...
    base_joint_vel = RewTerm(
        func=joint_vel_l2,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=DOG_JOINT_NAMES)},
        weight=-5.0e-5,  # Lower penalty for base movement
    )


@configclass
class MobileTerminationsCfg:
    """Termination configuration for mobile grasp."""
...
@configclass
class Go2X5VLASMobileEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for VLA-SFT Layer 2 (Mobile Grasp) environment.

    This environment supports scene-based data collection with:
    - Navigation to target objects (1.5-3m distance)
    - Obstacle avoidance scenarios
    - Table approach tasks
    - Partial occlusion handling
    - VLA-specific camera outputs
    """

    scene: Go2X5VLASMobileSceneCfg = field(
        default_factory=lambda: Go2X5VLASMobileSceneCfg(
            num_envs=32,  # Fewer envs due to larger workspace
            env_spacing=6.0,  # Larger spacing for navigation
            replicate_physics=True,
        )
    )

    observations: MobileObservationsCfg = field(default_factory=MobileObservationsCfg)
    actions: MobileActionsCfg = field(default_factory=MobileActionsCfg)
    rewards: MobileRewardsCfg = field(default_factory=MobileRewardsCfg)
    terminations: MobileTerminationsCfg = field(default_factory=MobileTerminationsCfg)
    events: MobileEventCfg = field(default_factory=MobileEventCfg)
    ...
    vla_sft: Dict[str, any] = field(
        default_factory=lambda: {
            ...
            "data_collection": {
                "output_dir": "./data/vla_sft/mobile",
                "save_images": True,
                "save_frequency": 1,
            },
        }
    ...
    def __post_init__(self):
        ...
        self.decimation = 4
        self.episode_length_s = 12.0  # Longer episodes for navigation
        self.sim.dt = 0.005
        ...
        self.viewer.eye = (4.0, 4.0, 3.0)  # Further back for larger workspace
        self.viewer.lookat = (0.0, 0.0, 0.4)


@configclass
class Go2X5VLASMobileEnvCfg_PLAY(Go2X5VLASMobileEnvCfg):
